...
dbw_dump_db(struct dbw_db *db)
{
    ...
    for (size_t kd = 0; kd < db->keydependencies->n; kd++) {
        struct dbw_keydependency *dep = (struct dbw_keydependency *)db->keydependencies->set[kd];
        printf("kd: %d, from: %d, to: %d\n", dep->id, dep->fromkey->id, dep->tokey->id);
    }
}
